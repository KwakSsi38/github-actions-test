...
def _get_content(
    ...
    for attempt in range(3):
        try:
            ...
            # rate limit 처리 (github_client의 로직과 동일하게) # TODO: 이건 그냥 동일하게가 아니라 가져다 쓰면 되는 거 아닌가?
...
def fetch_one(gid_str: str, index: dict, work_dir: Path) -> bool:
    # TODO: 단일 레포에 700개 이렇게 있는 경우도 있는데 이런 건 어떻게 처리해야 할지? 100개마다 저장할 수 있게 하는 게 낫지 않을까?
    """
